...
def set_cell_text(cell, text: str, bold: bool = False, font_size: float = 9):
...
def format_word_table(table, column_widths: list[float] | None = None, font_size: float = 9):
...
def add_markdown_table(doc: Document, table_lines: list[str]):
    ...
    table.style = "Table Grid"
    case_table_widths = [1.0, 2.0, 1.0, 1.0, 3.0] if max_cols == 5 else None
    table_font_size = 8 if max_cols >= 5 else 9

    for idx, value in enumerate(rows[0]):
        set_cell_text(table.rows[0].cells[idx], value, bold=True, font_size=table_font_size)
    ...
    format_word_table(table, column_widths=case_table_widths, font_size=table_font_size)
    doc.add_paragraph("")
